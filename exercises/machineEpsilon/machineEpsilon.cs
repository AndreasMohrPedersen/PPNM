static class main{
	...
	static void Main(){
		...
		double x=1; while(1+x!=1){x/=2;} x*=2; System.Console.Write($"machineEpsilon for double: {x}\n");
		
		System.Console.WriteLine($"2^(-23): {machineEpsilonFloat}");
		float y=1F; while((float)(1F+y) != 1F){y/=2F;} y*=2F; System.Console.Write($"machineEpsilon for float: {y}\n");
